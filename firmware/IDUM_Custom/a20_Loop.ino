...
void loop() {
  ...
  updateBreakClock();  //CUSTOM (F14/F6): advance the break tick + clock-locked sculpt phase every loop, independent of any active modification

  seedRNGOnFirstClock();  //re-seed the RNG from the timing of the first clock edge (runs once); must come before modifyTest draws random()
